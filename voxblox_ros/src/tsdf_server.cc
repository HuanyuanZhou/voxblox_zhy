...
void TsdfServer::getServerConfigFromRosParam(
  ...
  nh_private.param("min_time_between_msgs_sec", 
                    min_time_between_msgs_sec,
                    min_time_between_msgs_sec);
  min_time_between_msgs_.fromSec(min_time_between_msgs_sec);

  double min_time_between_loopClose_sec = 0.0;
  nh_private.param("min_time_between_loopClose_sec", 
                    min_time_between_loopClose_sec,
                    min_time_between_loopClose_sec);
  min_time_between_loopClose_.fromSec(min_time_between_loopClose_sec);
...
) 
{
  ...
  if (verbose_) {
    ROS_INFO(" %lu points.", points_C.size());
  }
...
) 
{
  ...
  else if(last_msg_time_ptcloud_ - pointcloud_msg_in->header.stamp > min_time_between_loopClose_)
  {
    pointcloud_queue_.push(pointcloud_msg_in);
    isDeintegrate = true;
    ROS_INFO("LoopClosing May Update in stamp %f, last stamp %f",
