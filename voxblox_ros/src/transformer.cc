...
bool Transformer::lookupTransformQueue(const ros::Time& timestamp,
  ...
  if (match_found) 
  {
    //ROS_INFO(it->transform.translation.x)
    ROS_INFO_STREAM("transform timestamp: " << it->header.stamp << std::endl);
    ROS_INFO_STREAM("pointcloud timestamp: " << timestamp << std::endl);
    ROS_INFO_STREAM(
      "translation: " << it->transform.translation.x << ' ' << it->transform.translation.y << ' ' << it->transform.translation.z << ' ' << std::endl;
    );
    ROS_INFO_STREAM(
      "rotation: " << it->transform.rotation.x << ' ' << it->transform.rotation.y << ' ' << it->transform.rotation.z << ' '  << it->transform.rotation.w << std::endl;
    );
