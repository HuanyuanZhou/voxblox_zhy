...
// Updates tsdf_voxel zhy. Thread safe.
void TsdfIntegratorBase::updateTsdfVoxelZhy(
  const Point& origin,
  const Point& point_G,
  const GlobalIndex& global_voxel_idx,
  const Color& color, const float weight,
...
) 
{
  ...
  while (index_getter->getNextIndex(&point_idx) &&
    ...
    while (ray_caster.nextRayIndex(&global_voxel_idx)) {
      ...
      const float weight = isDeintegrate ? -getVoxelWeight(point_C) : getVoxelWeight(point_C);

      updateTsdfVoxelZhy(origin, point_G, global_voxel_idx, color, weight, voxel);
    }
  }
}
